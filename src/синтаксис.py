...
class Парсер:
    ...
    def _функция(self):
        ...
        мандаты = []
        # синтаксис: функ имя(...) требует [Мандат, Мандат] { ... }
        if self._тек().вид == Вид.ИМЯ and self._тек().текст == "требует":
            self._съесть()
            self._съесть(Вид.МЕНЬШЕ) if False else None  # мандаты в [ ] обрабатываются как имена
            # ожидаем '[' ... ']' — лексер отдаёт их как ИМЯ? нет: используем простую форму
            # поддержим: требует Мандат1 Мандат2 до '{'
            while self._тек().вид == Вид.ИМЯ:
                мандаты.append(self._съесть().текст)
                if self._тек().вид == Вид.ЗАПЯТАЯ:
                    self._съесть()
        тело = self._блок()
