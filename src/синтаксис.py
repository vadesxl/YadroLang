...
class Парсер:
 МАКС=512
 def __init__(self,токены):self.т=токены;self.i=0;self.глубина=0
 ...
 def разобрать(self):
  ...
  while self._тек().вид!=Вид.КОНЕЦ:
   до=self.i;прог.функции.append(self._функция())
 ...
 def _блок(self):
  self._съесть(Вид.ЛФИГ);у=[]
  while self._тек().вид!=Вид.ПФИГ:
   if self._тек().вид==Вид.КОНЕЦ:raise ОшибкаПарсера("незакрытый блок")
   до=self.i;у.append(self._утверждение())
   if self.i<=до:raise ОшибкаПарсера("парсер не продвинулся")
  self._съесть(Вид.ПФИГ);return у
 def _утверждение(self):
  в=self._тек().вид
  if в==Вид.ВЕРНУТЬ:ст=self._съесть().строка;return Вернуть(ст,self._выражение())
  if в==Вид.ПУСТЬ:ст=self._съесть().строка;имя=self._съесть(Вид.ИМЯ).текст;self._съесть(Вид.РАВНО);return Пусть(ст,имя,self._выражение())
  if в==Вид.ЕСЛИ:return self._если()
  if в==Вид.ПОКА:ст=self._съесть().строка;усл=self._выражение();return Пока(ст,усл,self._блок())
  if в==Вид.ИМЯ and self.i+1<len(self.т) and self.т[self.i+1].вид==Вид.РАВНО:ст=self._тек().строка;имя=self._съесть().текст;self._съесть();return Присвоить(ст,имя,self._выражение())
  return self._выражение()
 def _если(self):
  ст=self._съесть(Вид.ЕСЛИ).строка;усл=self._выражение();тогда=self._блок();иначе=[]
 ...
 def _выражение(self,мин=0):
  ...
  try:
   ...
   while True:
    ...
    оп=self._съесть();справа=self._выражение(пр+1);слева=Бинарный(оп.строка,ОП[в],слева,справа)
   return слева
 ...
 def _первичное(self):
  ...
  if ток.вид==Вид.ЛСКОБ:self._съесть();в=self._выражение();self._съесть(Вид.ПСКОБ);return в
  if ток.вид==Вид.ИМЯ:
   self._съесть()
   if self._тек().вид==Вид.ЛСКОБ:
    self._съесть();а=[]
    while self._тек().вид!=Вид.ПСКОБ:
     а.append(self._выражение())
     if self._тек().вид==Вид.ЗАПЯТАЯ:self._съесть()
