...
MAX_ENTRY_POINTS=256
MAX_ASSUMPTIONS=2_000
MAX_SEMANTIC_SET=32
MAX_IDENTIFIER_BYTES=256
MAX_PATH_BYTES=512
SCHEMA="yadro-proof-seal-1.0"
POLICY_VERSION="yadro-policy-1.0"
...
def safe_path(value):
 ...
 parts=value.split("/")
 if any(part in ("",".","..") for part in parts):raise ProofSealError("module_path has unsafe segment")
 return value

@dataclass(frozen=True,slots=True)
class TrustState:
 mode:str="unsigned"
 authenticity:str="not-provided"
 def __post_init__(self):
  _enum(self.mode,"trust mode",{"unsigned"});_enum(self.authenticity,"authenticity",{"not-provided"})
...
class CompilerIdentity:
 ...
 evidence_algorithm:str="yadro-evidence-1.0"
 llvm_compatibility:str="llvmlite-0.43"
 def __post_init__(self):
...
class CallSiteEvidence:
 ...
 declassified_labels:tuple[str,...]
 policy_rules:tuple[str,...]
 implicit_labels:tuple[str,...]
 assumption_ids:tuple[str,...]
 reachable_entries:tuple[str,...]
 status:str="allowed"
 def __post_init__(self):
...
class AnalysisEvidence:
 ...
 call_sites:tuple[CallSiteEvidence,...]
 assumptions:tuple[ExternalAssumption,...]
 fixpoint:FixpointEvidence
 def __post_init__(self):
  _require_canonical_strings(self.entry_points,"entry_points",MAX_ENTRY_POINTS)
  if len(self.call_sites)>MAX_CALL_SITES:raise ProofSealError("too many call sites")
  if len(self.assumptions)>MAX_ASSUMPTIONS:raise ProofSealError("too many assumptions")
  ...
  if len({x.id for x in self.call_sites})!=len(self.call_sites):raise ProofSealError("duplicate call-site id")
  if len({x.id for x in self.assumptions})!=len(self.assumptions):raise ProofSealError("duplicate assumption id")
...
class SubjectBinding:
 ...
 llvm_normalization_version:str
 source_sha256:str
 policy_sha256:str
 llvm_sha256:str
 artifact_sha256:str
...
def _require_canonical_strings(value,name,max_items=MAX_SEMANTIC_SET):
 if not isinstance(value,tuple):raise ProofSealError(f"{name} must be tuple")
 expected=canonical_strings(value,name,max_items)
 if expected!=value:raise ProofSealError(f"{name} must be canonical")
...
def make_assumption(symbol,abi_signature,capability,taint_transform,trusted_sanitizer,lifetime,no_retain,implementation_sha256=None):
 payload={"abi_signature":_text(abi_signature,"ABI signature",512),"capability":None if capability is None else _text(capability,"capability"),"implementation_sha256":None if implementation_sha256 is None else _digest(implementation_sha256,"implementation digest"),"lifetime":_text(lifetime,"lifetime",512),"no_retain":no_retain,"symbol":_text(symbol,"symbol"),"taint_transform":_text(taint_transform,"taint transform",512),"trusted_sanitizer":trusted_sanitizer}
 ...
 identity=hashlib.sha256(b"YADRO-ASSUMPTION\0"+_canonical_mapping(payload)).hexdigest()
...
def make_call_site(id,caller,callee,span,**sets):
 ...
 values={name:canonical_strings(sets.get(name,()),name,MAX_ENTRY_POINTS if name=="reachable_entries" else MAX_SEMANTIC_SET) for name in names}
 assumption_ids=tuple(sets.get("assumption_ids",()))
 ...
 values["assumption_ids"]=tuple(sorted(assumption_ids))
 return CallSiteEvidence(id,caller,callee,span,**values)

def make_analysis(entry_points=(),call_sites=(),assumptions=(),fixpoint=None):
 calls=tuple(call_sites);assumps=tuple(assumptions)
 if len({x.id for x in calls})!=len(calls):raise ProofSealError("duplicate call-site id")
 if len({x.id for x in assumps})!=len(assumps):raise ProofSealError("duplicate assumption id")
...
def call_site_id(frontend,module_digest,caller,callee,ast_kind,start_byte,end_byte,ordinal):
 frontend=_enum(frontend,"frontend",{"ru","en"});_digest(module_digest,"module id")
 caller=_text(caller,"caller");callee=_text(callee,"callee");ast_kind=_text(ast_kind,"AST kind")
 start=_integer(start_byte,"start_byte");end=_integer(end_byte,"end_byte");order=_integer(ordinal,"ordinal")
 if end<start:raise ProofSealError("end_byte precedes start_byte")
 parts=(frontend,module_digest,caller,callee,ast_kind,str(start),str(end),str(order))
 return hashlib.sha256(b"YADRO-CALL-SITE\0"+b"\0".join(x.encode() for x in parts)).hexdigest()
...
def _core_mapping(core):
 return {"analysis":_plain(core.analysis),"compiler":_plain(core.compiler),"schema":core.schema,"subject":_plain(core.subject),"trust":_plain(core.trust)}
...
def seal(core):
 ...
 digest=hashlib.sha256(b"YADRO-PROOF-SEAL\0"+b"1.0\0"+canonical_bytes(core)).hexdigest()
 return ProofSeal(core,digest)
