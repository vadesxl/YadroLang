...
from src.типизация import ПроверкаТипов,ОшибкаТипов
...
def компилировать(исходник,выводить_ir=False):
 ast=Парсер(Лексер(исходник).токены()).разобрать();_проверить_уникальность_функций(ast);_проверить_точку_входа(ast);_проверить_вызовы(ast);_проверить_выражения(ast);ПроверкаТипов(ИСТОЧНИКИ,СТОКИ,САНИТАЙЗЕРЫ).проверить(ast);ЭтическийАнализатор().проверить(ast);ir=Кодоген().сгенерировать(ast)
