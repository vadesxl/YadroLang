...
ТАЙМАУТ_TOOL=30;ПРОФИЛИ_АРИФМЕТИКИ=frozenset({"default","checked"})
...
def _проверить_профиль(арифметика):
 if арифметика not in ПРОФИЛИ_АРИФМЕТИКИ:raise ОшибкаПрофиляАрифметики(f"Неизвестный arithmetic profile '{арифметика}'.")
...
def главная():
 if len(sys.argv)<2:print("Использование: python -m src.main файл.яд [--ir] [--checked-arithmetic]");raise SystemExit(1)
 try:
  исходник=open(sys.argv[1],encoding="utf-8").read();профиль="checked" if "--checked-arithmetic" in sys.argv else "default";ir=компилировать(исходник,"--ir" in sys.argv,профиль)
