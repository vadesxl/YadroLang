...
СИСТЕМНЫЕ_API = set(СТОКИ) | set(ИСТОЧНИКИ) | set(САНИТАЙЗЕРЫ) | {"печать"}


class ОшибкаСемантики(Exception):
    """Вызов неизвестной функции или неверное число аргументов."""


def _собрать_вызовы(узел, out):
    if isinstance(узел, Вызов):
        out.append(узел)
    словарь = getattr(узел, "__dict__", None)
    if not словарь:
        return
    for значение in словарь.values():
        if isinstance(значение, list):
            for э in значение:
                _собрать_вызовы(э, out)
        elif hasattr(значение, "__dict__"):
            _собрать_вызовы(значение, out)
...
def _проверить_вызовы(ast):
    ...
    for ф in ast.функции:
        вызовы = []
        for утв in ф.тело:
            _собрать_вызовы(утв, вызовы)
        for в in вызовы:
            if в.имя in арность:
                if len(в.аргументы) != арность[в.имя]:
                    raise ОшибкаСемантики(
                        f"Функция '{в.имя}' ожидает {арность[в.имя]} арг., "
                        f"получено {len(в.аргументы)}.")
            elif в.имя not in СИСТЕМНЫЕ_API:
                raise ОшибкаСемантики(f"Неизвестная функция '{в.имя}'.")
...
def _собрать_узлы(узел, тип, out):
    ...
    for значение in словарь.values():
        if isinstance(значение, list):
            for э in значение:
                _собрать_узлы(э, тип, out)
        elif hasattr(значение, "__dict__"):
            _собрать_узлы(значение, тип, out)
...
def _проверить_выражения(ast):
    for ф in ast.функции:
        числа, бинарные = [], []
        for утв in ф.тело:
            _собрать_узлы(утв, Число, числа)
            _собрать_узлы(утв, Бинарный, бинарные)
        for ч in числа:
            if not (I64_МИН <= ч.значение <= I64_МАКС):
                raise ОшибкаСемантики(
                    f"Числовой литерал {ч.значение} вне диапазона i64.")
        for б in бинарные:
            if б.оп == "/" and isinstance(б.справа, Число) and б.справа.значение == 0:
                raise ОшибкаСемантики("Деление на ноль.")
...
def компилировать(исходник: str, выводить_ir=False) -> str:
    ...
    _проверить_вызовы(ast)                 # все вызовы определены и с верной арностью
    _проверить_выражения(ast)              # деление на ноль, диапазон i64
...
def главная():
    ...
    except (ОшибкаТочкиВхода, ОшибкаСемантики, ЭтическаяОшибка, ОшибкаПарсера, ОшибкаЛексера) as e:
