...
def _проверить_профиль(арифметика):
 if арифметика not in ПРОФИЛИ_АРИФМЕТИКИ:raise ОшибкаСемантики(f"Неизвестный arithmetic profile '{арифметика}'.")
 ...
 for ф in ast.функции:
  ...
  for б in бинарные:
   ...
   if арифметика=="checked" and б.оп in "+-*":
    ...
    if значение is not None and not I64_МИН<=значение<=I64_МАКС:raise ОшибкаСемантики(f"Переполнение checked i64 (строка {б.строка}).")
