...
class ТестыProofSealDesign(unittest.TestCase):
 def schema(self):
  return json.loads((ROOT/"proof-seal.schema.json").read_text(encoding="utf-8"),object_pairs_hook=self.no_duplicates)
 @staticmethod
 ...
 def test_nested_objects_reject_unknown_fields(self):
  schema=self.schema();objects=[schema["properties"]["compiler"],schema["properties"]["subject"],schema["properties"]["analysis"],schema["$defs"]["span"],schema["$defs"]["callSite"],schema["$defs"]["assumption"],schema["$defs"]["fixpoint"]]
