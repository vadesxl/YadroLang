...
class ТестыProofSealDesign(unittest.TestCase):
 ...
 def test_versions_are_visible_before_hashing(self):
  subject=self.schema()["properties"]["subject"];self.assertIn("policy_schema_version",subject["required"]);self.assertIn("llvm_normalization_version",subject["required"]);self.assertEqual("yadro-policy-1.0",subject["properties"]["policy_schema_version"]["const"]);self.assertEqual("yadro-llvm-normalization-1.0",subject["properties"]["llvm_normalization_version"]["const"])
