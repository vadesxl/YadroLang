...
def rich_core(order=False):
 ...
 span=SourceSpan("src/пример.яд",1,4,0);mid=module_id("ru",b"source")
 values=("ПДн","Здоровье") if order else ("Здоровье","ПДн")
 call=make_call_site("ru",mid,"call","caller","callee",span,required_capabilities=("ДоступСети",),incoming_labels=values,assumption_ids=(assumption.id,),reachable_entries=("entry",))
 compiler=CompilerIdentity("yadro-guard","2.1.0","ru","1.0")
