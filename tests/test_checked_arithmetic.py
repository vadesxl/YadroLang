...
class ТестыCheckedArithmetic(unittest.TestCase):
 ...
 def test_checked_intrinsics(self):
  for op,name in (("+","sadd"),("-","ssub"),("*","smul")):
   text=self.verified(f"функ calc(x) {{ вернуть x {op} 2 }} функция старт() {{ вернуть calc(3) }}".replace("функция","функ"))
 ...
 def test_boundaries(self):
  self.verified("функ старт() { вернуть 9223372036854775807 + 0 }");self.verified("функ старт() { вернуть -9223372036854775808 + 0 }")
